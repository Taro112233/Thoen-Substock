// app/admin/personnel-types/components/PersonnelTypeCreateDialog.tsx
// Personnel Types - Create Dialog Component (FIXED VERSION)
// Dialog สำหรับสร้างประเภทบุคลากรใหม่ - แก้ไข TypeScript errors
...
import { PERSONNEL_HIERARCHIES, PERMISSION_LABELS, type PersonnelTypeFormData, type PersonnelHierarchy } from '../types/personnel-type';

// FIXED: Form validation schema with all required boolean fields
const createPersonnelTypeSchema = z.object({
  typeCode: z.string()
...
type CreateFormData = z.infer<typeof createPersonnelTypeSchema>;

interface PersonnelTypeCreateDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

export function PersonnelTypeCreateDialog({
  open,
  onOpenChange,
  onSuccess
}: PersonnelTypeCreateDialogProps) {
  const [loading, setLoading] = useState(false);
  const [responsibilityInput, setResponsibilityInput] = useState('');
  const [responsibilities, setResponsibilities] = useState<string[]>([]);

  // FIXED: Explicitly typed useForm hook
  const {
    register,
    handleSubmit,
    watch,
    ...
    resolver: zodResolver(createPersonnelTypeSchema),
    defaultValues: {
  ...
  const onSubmit = async (data: CreateFormData) => {
    try {
      ...
      const formData: PersonnelTypeFormData = {
        ...data,
      ...
      const response = await fetch('/api/admin/personnel-types', {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        toast.error(result.error || 'เกิดข้อผิดพลาดในการสร้างข้อมูล');
      }
    } catch (error) {
      console.error('Error creating personnel type:', error);
      ...
      <DialogContent className="max-w-4xl max-h-[90vh]">
        <DialogHeader>
          <DialogTitle className="flex items-center space-x-2">
            ...
            <span>เพิ่มประเภทบุคลากรใหม่</span>
          </DialogTitle>
          <DialogDescription>
            สร้างประเภทบุคลากรใหม่พร้อมกำหนดสิทธิ์และหน้าที่ความรับผิดชอบ
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="typeCode"
                    placeholder="เช่น DIR, GRP_HEAD, STAFF"
                    {...register('typeCode')}
                  ...
                  <p className="text-xs text-gray-500">
                    ใช้ตัวพิมพ์ใหญ่, ตัวเลข และ _ เท่านั้น
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="typeName"
                    placeholder="เช่น ผู้อำนวยการ, หัวหน้ากลุ่มงาน"
                    {...register('typeName')}
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="typeNameEn"
                    placeholder="เช่น Director, Group Head"
                    {...register('typeNameEn')}
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="เลือกระดับลำดับชั้น" />
                    </SelectTrigger>
                    <SelectContent>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {Object.entries(PERMISSION_LABELS).map(([key, config]) => (
                  <div key={key} className="flex items-start space-x-3 p-3 rounded-lg border">
                    <Checkbox
                      id={key}
                      checked={watch(key as keyof CreateFormData) as boolean}
                      onCheckedChange={(checked) => setValue(key as keyof CreateFormData, !!checked)}
                    />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="เช่น PHARMACY, ADMINISTRATION"
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  id="description"
                  placeholder="อธิบายหน้าที่และความรับผิดชอบของประเภทบุคลากรนี้"
